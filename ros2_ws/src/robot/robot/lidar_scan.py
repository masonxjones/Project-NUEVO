...
def init_avoidance_planner(robot: Robot) -> None:
    """
    Set up PurePursuitPlannerWithAvoidance on robot via the TA's API,
    then load LEG1B path via _set_obstacle_avoidance_path (correct legacy API).

    LEG1B starts 200mm ahead of the corridor entry so the planner's internal
    trimming never discards the first waypoint before the robot reaches it,
    which would cause a reverse/spin.
    """
    ...
    # Start 200 mm ahead of entry so no waypoint is immediately behind the robot
    leg1b = [
        (1325.0,  810.0),   # 200 mm north of entry — safely ahead
        (1325.0, 3460.0),
        (2000.0, 3460.0),
    ]
