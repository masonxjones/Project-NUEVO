...
#             # Disable lidar avoidance once past (1325, 3460)
#             if current_x >= 1325.0 and current_y >= 3460.0:
#                 if hasattr(robot.planner, 'obstacle_avoidance'):
...
LOOKAHEAD_DIST  = 100.0   # mm
MAX_LINEAR_VEL  = 150.0   # mm/s
MAX_ANGULAR_VEL = 1.5     # rad/s
GOAL_TOLERANCE  = 20.0    # mm

# ---------------------------------------------------------------------------
# Corridor leg parameters (no avoidance — pure waypoint navigation)
# ---------------------------------------------------------------------------
...
AVOID_MAX_ANGULAR   = 1.5
AVOID_GOAL_TOL      = 20.0
# AVOID_OBS_RANGE     = 300.0   # commented out — lidar disabled
# AVOID_VIEW_ANGLE    = math.radians(70.0)
# AVOID_SAFE_DIST     = 200.0
# AVOID_DELAY         = 150
# AVOID_ALPHA_LD      = 0.7
# AVOID_OFFSET        = 270.0
# AVOID_LANE_WIDTH    = 500.0
...
def configure_robot(robot: Robot) -> None:
    ...
    robot.set_tracked_tag_id(TAG_ID)
    robot.enable_vision()
    # robot.enable_lidar()   # lidar disabled
...
def make_corridor_planner() -> PurePursuitPlanner:
    """Plain Pure Pursuit planner for the corridor — no lidar/avoidance."""
    return PurePursuitPlanner(
        lookahead_dist=AVOID_LOOKAHEAD,
        max_angular=AVOID_MAX_ANGULAR,
        goal_tolerance=AVOID_GOAL_TOL,
    )


def pp_tick(robot: Robot, planner: PurePursuitPlanner, remaining_path: list) -> tuple[list, bool]:
    """One Pure Pursuit tick. Returns (updated_path, goal_reached)."""
    current_x, current_y, current_theta_deg = robot.get_pose()
    current_theta_rad = math.radians(current_theta_deg)
...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        if state == "IDLE":
            robot.stop()
            show_idle_leds(robot)
            # robot._draw_lidar_obstacles()   # lidar disabled
        ...
        # ====================================================================
        # MOVING  — plain Pure Pursuit, LEG1A
        # ====================================================================
        elif state == "MOVING":
            show_moving_leds(robot)
            ...
            if (
                ...
                if len(remaining_path) <= 1:
                    print("[NAV] LEG1A complete — switching to corridor planner.")
                    robot.stop()
                    planner        = make_corridor_planner()
        ...
        # ====================================================================
        # MOVING_AVOID  — corridor leg, pure waypoints (no lidar)
        # ====================================================================
        elif state == "MOVING_AVOID":
            show_moving_leds(robot)
            ...
            remaining_path = robot._advance_remaining_path(
                remaining_path, current_x, current_y,
                advance_radius_mm=AVOID_LOOKAHEAD,
            )

            if len(remaining_path) <= 1:
                print("[NAV] Corridor complete — arrived at scanning station (2000, 3460).")
                robot.stop()
                checkpoint_stop_start = time.monotonic()
                state = "CUSTOMER_CHECKPOINT"
            else:
        ...
        # ====================================================================
            ...
            if time.monotonic() - checkpoint_stop_start >= 3.0:
                print(f"[DECISION] Target: Customer {detected_customer.upper()}")
                target_x, target_y = DELIVERY_LOCATIONS[detected_customer]
                print(f"[NAV] Routing via (2000, 3460) → ({target_x}, {target_y})")

                current_x, current_y, _ = robot.get_pose()
                planner        = make_planner()
                remaining_path = densify_polyline(
                    [(current_x, current_y), (2000.0, 3460.0), (target_x, target_y)],
                    spacing=20.0,
                )
                checkpoint_done = True
                state = "DELIVERY_MOVING"
        ...
        # ====================================================================
            ...
            if len(remaining_path) <= 1:
                ...
                planner        = make_planner()
                remaining_path = densify_polyline(
                    [(current_x, current_y), (2000.0, 0.0)], spacing=20.0
                )
                stop_sign_detected = False
                state = "FINAL_MOVING"
            else:
